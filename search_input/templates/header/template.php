...
<div class="header-block_search larshin_search" > 
	...
	$.ajax({
		...
		data: 'search='+data,
			success: function(html){ 
				...
				history.pushState(null, null, '/search/?search='+data+'');
			}
	});
}
Search.load_search_from_enter=function(){
		...
		if(keyCode == 13)
		{
			var search_status=$('.search_result').attr('data-search');
			if($('.search_input_form_text').val()!=''){
				if(search_status=='Y'){
					Search.load_light_result($('.search_input_form_text').val()); 
				}else{
					window.location.href = "/search/?search="+$('.search_input_form_text').val()+"";
				}
			}
		}
}
$(document).ready(function(){
	/*var search_status = null;
	$('body').on('keypress', '.search_input_form_text', function(event){
		...
		if(search_status=='Y'){
			...
			window.location.href = "/search/"+$('.search_input_form_text').val()+"/";
		}
	});*/
